														...
														v-loading="loading">
															<thead class="thead-inverse">
																<tr>
																	...
																	<th width="130">Total Sales Amount</th>
															...
															<tbody>
																		...
																		<td>{{model.name}}</td>
																		<td>{{model.sale_price| NumberFormat}}</td>
	...
	export default {
		...
		methods: {
			...
			dataFetch(page) {
				var me = this;
				me.loading = true;
				saleService.search({page:page,search:me.search, status: true}).then(function(response){  
			...
			deletemodel(hash_id) {
				let me = this;
				 me.$confirm('This will permanently delete the data. Continue?', 'Warning', {
			          confirmButtonText: 'OK',
			          cancelButtonText: 'Cancel',
			          type: 'warning'
			    }).then(() => {
			        invoiceService.destroy(hash_id).then(function(response){  
						me.dataFetch(me.pagenation.currentPage);
				    }).catch(function(response){
				           console.log(response)
				    });
			...
			showmodel(hash_id) {
				location.href=`/sales/show/${hash_id}`;
			},
			sendToQueue(id) {
